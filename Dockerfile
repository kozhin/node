...
RUN addgroup -g 1000 node \
    ...
    && apk add --no-cache --virtual .build-deps \
        ...
        make \
        python3 \
# BUGFIX: Yarn download fails at Github.com, so DNS were changed to avoid that.
    && echo "nameserver 8.8.8.8" > /etc/resolv.conf \
# Install NodeJS
# NodeJS's gpg keys listed at https://github.com/nodejs/node#release-keys
    && for key in \
